...
router.beforeEach((to, from, next) => {
    if (to.matched.some((record) => record.meta.requiresEditPublishPermissions)) {
        store.dispatch('GetReport', to.params.id).then(() => {
            if (store.state.reports.report.user.id === store.state.auth.user.id && store.getters.isAnnouncer) {
                next();
                return
            }
            next('/403');
