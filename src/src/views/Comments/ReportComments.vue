  ...
  <div v-else>
    <v-card class="mt-4">
      <v-card-text class="grey--text text--darken-2">
        COMMENTS <p class="d-inline teal--text font-weight-medium">{{ comments.length }}</p>
      </v-card-text>
      <v-divider></v-divider>
      <v-list
          align-top
...
export default {
  ...
  methods: {
    ...
    sendComment() {
      ...
      let newDate = year + '-' + month + '-' + day + ' ' + hr + ':' + min + ':' + sec
      if (this.comment) {
        const form = {
      ...
      this.comment = null
      this.content = ''
      this.GetComments(this.$route.params.id).then(() => this.loading = false)
    }
  },
  created() {
    this.GetComments(this.$route.params.id).then(() => this.loading = false)
